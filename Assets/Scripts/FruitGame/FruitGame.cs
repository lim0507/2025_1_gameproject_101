...
public class FruitGame : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(currentFruit != null)
        {
            ...
            if(newPosition.z > gameWidth/2 + halfFruitSize)
            {
                newPosition.x = gameWidth / 2 + halfFruitSize;
            }
    ...
    void SpawnNewFruit()
    {
        if(!isGameOver)
        {
            ...
            Vector3 mousesPosition = Input.mousePosition;
            Vector3 worldPosition = mainCamera.ScreenToWorldPoint(mousesPosition);
